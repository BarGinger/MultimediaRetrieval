...
class UnifiedPreprocessingProcessor:
    ...
    def apply_remeshing_if_needed(self, mesh_path, target_vertices, tolerance=0.2):
        ...
        try:
            ...
            # Clean mesh - USE MINIMAL CLEANING to preserve connectivity
            # Aggressive cleaning (remove_degenerate_triangles, remove_non_manifold_edges)
            # can break meshes apart into many disconnected components
    ...
    def _upsample_to_range(self, mesh):
        ...
        # For very low-poly meshes (< 500 vertices), be more conservative
        if original_vertex_count < 500:
            msg = f"Very low-poly mesh ({original_vertex_count} vertices), using conservative upsampling"
            print(f"    ⚠️  {msg}")
            upsampling_log['warnings'].append(msg)
            max_subdiv_passes = 2  # Limit to 2 passes for safety
            max_upsampling_factor = 5  # Only allow 5x increase
            max_allowed_vertices = original_vertex_count * max_upsampling_factor
        ...
        if final_vertex_count < self.min_acceptable_vertices:
            ...
            print(f"    ⚠️  {msg}")
            print(f"    ⚠️  Preserving mesh quality over vertex count")
            upsampling_log['warnings'].append(msg)
            upsampling_log['warnings'].append("Preserving mesh quality over vertex count target")
