    ...
    def _get_analysis_path(self, dataset: str) -> Optional[str]:
        ...
        path_mapping = {
            ...
            'UnifiedPreprocessed/Data': [
                'Datasets/UnifiedPreprocessed/Data/analysis_results_unifiedPreProcessed_data.csv',
        ...
        if dataset == 'UnifiedPreprocessed/Data':
            a = "1"
...
def merge_analysis_data(file_df: pd.DataFrame, dataset: str) -> pd.DataFrame:
    ...
    if analysis_df is not None:
        ...
        if 'UnifiedPreprocessed' in dataset:
            ...
            file_df_copy['fallback_filename'] = file_df_copy['filename'].str.replace(r'_\d+_.*\.obj$', '.obj', regex=True)
            analysis_df_copy = analysis_df.copy()
            analysis_df_copy['fallback_filename'] = analysis_df_copy['filename'].str.replace(r'_\d+_.*\.obj$', '.obj', regex=True)

            # Merge using fallback filenames
            merged = pd.merge(
                file_df_copy,
                analysis_df_copy[['category', 'filename', 'fallback_filename', 'num_vertices', 'num_faces']],
                left_on=['category', 'filename'],
                right_on=['category', 'fallback_filename'],
                how='left'
            )

            # Update unmatched entries with fallback matches
            unmatched_mask = merged['num_vertices'].isna()
            if unmatched_mask.any():
                print(f"[DEBUG] Fallback matching for {unmatched_mask.sum()} entries...")
                merged.loc[unmatched_mask, ['num_vertices', 'num_faces']] = pd.merge(
                    file_df_copy[unmatched_mask],
                    analysis_df_copy[['category', 'fallback_filename', 'num_vertices', 'num_faces']],
                    left_on=['category', 'fallback_filename'],
                    right_on=['category', 'fallback_filename'],
                    how='left'
                )[['num_vertices', 'num_faces']].values
        ...
        print(f"[DEBUG] No analysis data available for dataset: {dataset}")
        return file_df
